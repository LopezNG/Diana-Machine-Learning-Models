...
# "2021-2023" -> cycle_start = 2021, cycle_end = 2023
...
has_outlier = df['has_outlier'].copy()          # keep flag for later clamping
...
# Compute IQR bounds from NON-outlier training rows
...
#    Stratified 10-Fold CV | oob_score = True
# ============================================================
print("\n" + "=" * 65)
...
param_dist = {
    ...
    'max_samples':       uniform(0.5, 0.5),   # 50%-100% of training set
}

rf_base = RandomForestClassifier(oob_score=True, random_state=RANDOM_STATE, n_jobs=-1)
...
# ============================================================
val_metrics = evaluate(best_rf, X_val_sc, y_val, "Validation Set")

# ============================================================
# 11. BAGGING - TEST SET EVALUATION
# ============================================================
print("\n" + "=" * 65)
print("11. BAGGING - TEST SET (Final Model Performance)")
print("=" * 65)

n_trees = len(best_rf.estimators_)
print(f"\nThe tuned Random Forest contains {n_trees} bagged decision trees.")
print("Each tree independently predicts the test samples; the ensemble")
print("aggregates via majority vote to produce the final predictions.\n")

# --- Show the bagging mechanism explicitly ---
tree_preds = np.array([tree.predict(X_test_sc.values)
                        for tree in best_rf.estimators_])
print(f"Individual tree predictions matrix: {tree_preds.shape}  (trees x samples)")

bagged_preds = mode(tree_preds, axis=0)[0].flatten()
model_preds  = best_rf.predict(X_test_sc)
agree = np.mean(bagged_preds == model_preds)
print(f"Manual bagging <-> model.predict() agreement: {agree:.4f}")

# Full evaluation on the held-out test set
test_metrics = evaluate(best_rf, X_test_sc, y_test, "Test Set (Bagged)")
...
print(f"  Num Trees   : {best_rf.n_estimators}")
