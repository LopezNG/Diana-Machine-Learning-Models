...
#    Stratified 10-Fold CV  |  XGBoost
# ============================================================
print("\n" + "=" * 65)
print("9. HYPERPARAMETER TUNING - RandomizedSearchCV (XGBoost)")
print("=" * 65)
...
param_dist = {
    ...
    'subsample':         uniform(0.5, 0.5),        # 50%-100%
    'colsample_bytree':  uniform(0.5, 0.5),        # 50%-100%
    'gamma':             uniform(0, 5),
    'reg_alpha':         loguniform(1e-3, 10),
    'reg_lambda':        loguniform(1e-3, 10),
    'min_child_weight':  randint(1, 10),
    'scale_pos_weight':  [1],                      # handled by multi-class
}
...
def evaluate(model, X_data, y_data, set_name):
    print(f"\n{'=' * 65}")
    print(f"XGBoost - EVALUATION - {set_name}")
    print(f"{'=' * 65}")
...
# ============================================================
val_metrics = evaluate(best_xgb, X_val_sc, y_val, "Validation Set")

# ============================================================
# 11. BOOSTING - TEST SET EVALUATION
# ============================================================
print("\n" + "=" * 65)
print("11. XGBoost - TEST SET (Final Model Performance)")
print("=" * 65)

n_trees = best_xgb.n_estimators
print(f"\nThe tuned XGBoost model contains {n_trees} boosted trees.")
print(f"Trees are built sequentially; each new tree corrects the")
print(f"residual errors of the ensemble so far (gradient boosting).")
print(f"  Learning rate  : {best_xgb.learning_rate:.6f}")
print(f"  Max depth      : {best_xgb.max_depth}")
print(f"  Subsample      : {best_xgb.subsample:.4f}")
print(f"  Col-sample     : {best_xgb.colsample_bytree:.4f}")
...
test_metrics = evaluate(best_xgb, X_test_sc, y_test, "Test Set (Boosted)")

# ============================================================
# 12. FINAL SUMMARY TABLE
# ============================================================
print("\n" + "=" * 65)
print("12. XGBoost - FINAL SUMMARY")
print("=" * 65)

summary_df = pd.DataFrame({'Validation': val_metrics,
                            'Test (Boosted)': test_metrics}).T
print(f"\n{summary_df.round(4).to_string()}")

print(f"\nBest Hyperparameters:")
for k, v in search.best_params_.items():
    print(f"  {k:22s}: {v}")
print(f"\n  Num Trees    : {best_xgb.n_estimators}")
